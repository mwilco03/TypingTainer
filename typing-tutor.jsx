...
const KEY_TO_FINGER = {
  'q': 'leftPinky', 'a': 'leftPinky', 'z': 'leftPinky', '1': 'leftPinky',
  ...
  'g': 'leftIndex', 'b': 'leftIndex', '4': 'leftIndex', '5': 'leftIndex',
  'y': 'rightIndex', 'h': 'rightIndex', 'n': 'rightIndex', 'u': 'rightIndex',
  'j': 'rightIndex', 'm': 'rightIndex', '6': 'rightIndex', '7': 'rightIndex',
...
const Key = ({ letter, isActive, isPressed, isInLesson, showFingerHint }) => {
  ...
  const baseClasses = "flex items-center justify-center font-bold rounded-lg transition-all duration-100 select-none";
  const sizeClasses = letter === ' ' ? "w-48 h-12" : "w-12 h-12";
  
  const getBgColor = () => {
    if (isPressed) return 'bg-blue-400 scale-95';
    if (isActive) return `bg-opacity-100`;
    if (!isInLesson) return 'bg-gray-300 opacity-50';
    return 'bg-gray-200';
  };

  return (
    <div
      className={`${baseClasses} ${sizeClasses} ${getBgColor()} border-2 ${isHomeRow ? 'border-blue-500' : 'border-gray-400'}`}
      style={{
    ...
    >
      ...
      {isHomeRow && (
        <div className="absolute -bottom-1 w-2 h-1 bg-blue-500 rounded-full" />
      )}
    </div>
  );
};
...
const LessonCard = ({ lesson, isCompleted, isCurrent, onSelect }) => {
  return (
    <button
      onClick={() => onSelect(lesson)}
      className={`
        p-4 rounded-xl text-left transition-all w-full
        ${isCurrent ? 'bg-blue-100 border-2 border-blue-500' : 'bg-white border-2 border-gray-200'}
...
export default function TypingTutor() {
  ...
  }, [screen, currentText, currentIndex, currentLesson, exerciseIndex, startTime, accuracy, wpm, unlockAchievement]);
      ...
      <div className="min-h-screen bg-gradient-to-b from-blue-100 to-purple-100 p-8">
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-5xl font-bold text-blue-600 mb-2">⌨️ Type Quest!</h1>
            <p className="text-xl text-gray-600">Learn to type like a pro!</p>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-purple-50 p-4">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-6 text-center text-gray-400 text-sm">
          💡 Tip: Keep your fingers on the home row (ASDF JKL;) and feel for the bumps on F and J!
        </div>
      </div>
    </div>
  );
